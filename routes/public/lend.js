...
router.get('/all/:userId', async (req, res) => {
    try {
        ...
        const response = await lendService.getLends(userId);
...
router.post('/addLend', async (req, res) => {
    try {
        ...
        const {userId, name, title, amount, lendDate, status, remarks} = req.body;
        if(!userId || !name || !title || !amount || !lendDate || !status){
            return res.status(400).send({
                status: 'error',
        ...
        const response = await lendService.addLend(userId, record);
        if (response.status === 'success') {
            res.status(200).send(response);
        } else {
            res.status(400).send(response);
